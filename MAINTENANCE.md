...
plus `make -n install` before authorized publication. For CONTACTS-001, the missing
isolated behavioral fixture blocks source-sync success (not merely publication) until
the required mock acceptance procedure passes. Update this register with any patch
addition/change/retirement; missing or stale coverage blocks publication.
